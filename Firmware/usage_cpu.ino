...
void CPUUsageVisualiser(char *packetBuffer) {
  ...
  switch (cmd) {
    ...
    default:
      ...
      if (FastLED.getBrightness() != BR) FastLED.setBrightness(BR);
      break;
  }
...
void usageSpringLevel(byte cmd, byte level, byte bar_h, byte colorDisplay) {
  ...
  const byte pixStep = ceil( (0.1 + NUM_LEDS) / 160.0);
  uint16_t lvl;
  // ------------------------------------
  lvl = (level * NUM_LEDS) / 100.0;
  if (lvl > ff_x) {
    ff_x += pixStep;
  } else {
    if (ff_x > 0U)
      if (lvl < ff_x) ff_x -= pixStep;
  }

  for (uint8_t y = 0U; y < HEIGHT; y++) {
    for (uint8_t x = 0U; x < WIDTH; x++) {
      uint16_t i = x + y * WIDTH;
