...
void testMatrix(uint8_t val) {
  if (val == 1) {
    ...
    FastLED.show();
#ifdef USE_ROBOT_007
    showDebugMsg("TEST 1", "MATRIX", 3000);
#endif;
  } else {
    drawRec(0, 0, WIDTH, HEIGHT, 0xFF0000);
    FastLED.show();
#ifdef USE_ROBOT_007
    showDebugMsg("TEST 2", "MATRIX", 3000);
#endif;
    delay(2000);
    drawRec(0, 0, WIDTH, HEIGHT, 0x00FF00);
    FastLED.show();
#ifdef USE_ROBOT_007
    showDebugMsg("TEST 3", "MATRIX", 3000);
#endif;
    delay(2000);
    drawRec(0, 0, WIDTH, HEIGHT, 0x0000FF);
    FastLED.show();
#ifdef USE_ROBOT_007
    showDebugMsg("TEST 4", "MATRIX", 3000);
#endif;
    delay(2000);
    if (WIDTH > HEIGHT) {
    ...
    FastLED.show();
#ifdef USE_ROBOT_007
    showDebugMsg("TEST 5", "MATRIX", 3000);
#endif;
    delay(5000);
    FastLED.clear();
...
String runCommand(byte cmd, uint8_t val, String valStr) {
  ...
  switch (cmd ) {
    ...
    case CMD_PREV_EFF:
      ...
      //#ifdef USE_ROBOT_007
      //      if (robot_manual_mode) {
      //        showPlayerCommand("PREV");
      //        send_command(0x02, FEEDBACK, 0, 21);
      //      } else {
      //        selectedSettings = 0U;
      //        prevEffect();
      //      }
    ...
    case CMD_NEXT_EFF:
      ...
      //#ifdef USE_ROBOT_007
      //      if (robot_manual_mode) {
      //        showPlayerCommand("NEXT");
      //        send_command(0x01, FEEDBACK, 0, 21);
      //      } else {
      //        selectedSettings = 0U;
      //        nextEffect();
      //      }
    ...
    case CMD_BRIGHT_UP:
      ...
      if (robot_manual_mode) {
        ...
        if (headAngle < 170) {
          headAngle += 10;
          sleep_mode = false;
          //          servoHead.setTargetDeg(headAngle);
    ...
    case CMD_RANDOM:
      ...
      updateSets();
#ifdef USE_ROBOT_007
      //      servoHead.setTargetDeg(random8(30, 150));
      HeadRotation(random8(30, 150));
#endif
      break;
    case CMD_LIST:
      loadingFlag = false;
...
#ifdef USE_ROBOT_007
  byte iot = IOT_TYPE + (isServo ? 8 : 0);
  lamp_state += "\"javelin\":" + String(iot) + ",";
#else
